...
cold_hash <- "sha256:5c4065fa5ff7baa4abae80e28d0d0643714f425ac3c62e560c2b309a7b7ba2f3"
...
assert_true(
  ...
  "N4 v3 may consume only the authorized frozen N2 bytes."
)
...
all_claim_checks <- paste0(
  ...
  c(
    ...
    "six-cell coverage", "v2 provenance boundary", "directed negative fixtures"
  )
)
...
oracle_specification <- list(
  implementation = "scripts/oracle_essential_input_n4_v3.R",
...
ballot_response_map <- function(m_class, prior_region) {
  list(
    ...
    weak_veto_bounds = "A sole weak veto k requires x_ik<=C, including equality; with at least two weak vetoes and m>=3 no additional payment bound exists beyond feasibility.",
...
branch_coverage <- function(m_class, prior_region) {
  ...
  list(
    ...
    at_least_two_weak_vetoes = if (m_class == "mge3") {
      list(status_rule = "exists for every prior and every feasible package", none_certificate = NULL)
    } else {
      list(
        status_rule = "none",
        none_certificate = none_certificate(
  ...
  list(
    ...
    multiple_weak_vetoes = if (m_class == "mge3") {
      list(
        ...
        condition = "No payment restriction beyond proposal feasibility; identity and every linked failure-vector belief remain part of the source assessment."
      )
    } else {
      list(
...
make_claim <- function(index, branch, claim, evidence_section, status = "proved") {
  list(
    claim_id = sprintf("N4V3-CLM-%03d", index),
    ...
    status = status,
    evidence = paste0(
      "model_redesign/essential_input_n4_r1_unanimity_derivation_v3.md#",
      evidence_section
    )
  )
}
...
claims <- list(
  ...
  make_claim(12, "multi_veto", "For m>=3 every labeled set of at least two weak vetoes is sustainable at every prior and feasible package; m=2 has no such set.", "respostas-weak-exatas"),
  ...
  make_claim(20, "provenance", "The sealed cold derivation preserves structural invariants from v2 but replaces every invalid security, cap, endpoint, delay, and multi-veto conclusion listed in the provenance comparison.", "comparacao-pos-selagem-com-n4-v2"),
  make_claim(21, "negative_fixtures", "Independent executable fixtures validate the two review counterexamples and reject high-prior H-veto with x<B, Bayes corruption, missing vectors, wrong N2 records, and unauthorized primitives.", "oracle-vetorial-do-ballot", "checked numerically")
)
...
build_outputs <- function(interface_target, ledger_target) {
  ...
  ledger <- list(
    ...
    node_id = "N4",
    artifact_path = "model_redesign/essential_input_interfaces/n4_r1_unanimity_candidate_v3.json",
    artifact_hash = paste0("sha256:", interface_hash),
    cold_note_path = "model_redesign/essential_input_n4_r1_unanimity_cold_notes_v3.md",
...
if (check_mode) {
  interface_tmp <- tempfile("n4-v3-interface-", fileext = ".json")
  ledger_tmp <- tempfile("n4-v3-ledger-", fileext = ".json")
  on.exit(unlink(c(interface_tmp, ledger_tmp)), add = TRUE)
  hashes <- build_outputs(interface_tmp, ledger_tmp)
  assert_true(file.exists(interface_path), "Canonical N4 v3 interface is missing.")
  assert_true(file.exists(ledger_path), "Canonical N4 v3 ledger is missing.")
  assert_true(
    identical_bytes(interface_tmp, interface_path),
    "Canonical N4 v3 interface is not build-stable."
  )
  assert_true(
    identical_bytes(ledger_tmp, ledger_path),
    "Canonical N4 v3 ledger is not build-stable."
  )
  cat("PASS: N4 v3 build is byte-stable\n")
  ...
  cat("PASS: built N4 v3 candidate artifacts\n")
  cat("interface_sha256=", hashes$interface_hash, "\n", sep = "")
  cat("ledger_sha256=", hashes$ledger_hash, "\n", sep = "")
}
