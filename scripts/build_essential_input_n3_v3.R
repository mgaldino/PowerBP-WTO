...
make_n3_v3_objects <- function(n1_hash) {
  common_domain <- c(
  ...
  make_record <- function(region) {
    ...
    list(
      ...
      checks_performed = c(
        "N3V3-C01 frozen N1 import and exactly-one discount",
        "N3V3-C02 weak voting cutoff and T^Y",
        "N3V3-C03 complete H best-response map",
        "N3V3-C04 proposer payoff map for every feasible proposal",
        "N3V3-C05 exhaustive reduction to exclusion, low-type-only, pooling, and deliberate failure",
        "N3V3-C06 P0 full-pie use through belief-invariant N1 response maps",
        "N3V3-C07 P1 strict hedge dominance and P1a",
        "N3V3-C08 deliberate failure strictly dominated because 1-beta*q/m>0",
        "N3V3-C09 candidate feasibility in every selected cell",
        "N3V3-C10 strict-region and equality-boundary partition",
        "N3V3-C11 o_1=1/m residual proposer tie and expected-H tie-break",
        "N3V3-C12 pure identity assignments and all proposer mixtures",
        "N3V3-C13 Bayes and complete off-path proposal-vote beliefs at every nu",
        "N3V3-C14 P7 public H vote and posterior update",
        "N3V3-C15 identity-indexed weak payoff map",
        "N3V3-C16 closed N6-transported fields without free symbols",
        "N3V3-C17 complete existence and endpoint coverage"
      ),
  ...
  regions <- list(
    list(
      cell_id = "N3V3-CELL-O1LT-LOW",
      equilibrium_id = "N3V3-EQ-O1LT-LOW",
      branch = "low",
    ...
    list(
      cell_id = "N3V3-CELL-O1LT-POOL",
      equilibrium_id = "N3V3-EQ-O1LT-POOL",
      branch = "pool",
      conditions = c("o_1<1/m", paste0(nu_sp, "<nu<=1")),
      classification = "pooling R1 passage with H"
    ),
    list(
      cell_id = "N3V3-CELL-CROSS-LOW",
      equilibrium_id = "N3V3-EQ-CROSS-LOW",
      branch = "low",
    ...
    list(
      cell_id = "N3V3-CELL-CROSS-EXCLUDE",
      equilibrium_id = "N3V3-EQ-CROSS-EXCLUDE",
      branch = "exclude",
      conditions = c("o_0<1/m<o_1", paste0(nu_se, "<nu<=1")),
      classification = "R1 passage without H at y=0"
    ),
    list(
      cell_id = "N3V3-CELL-O0GT-EXCLUDE",
      equilibrium_id = "N3V3-EQ-O0GT-EXCLUDE",
      branch = "exclude",
      conditions = c("1/m<o_0<o_1", "0<=nu<=1"),
      classification = "R1 passage without H at y=0"
    ),
    list(
      cell_id = "N3V3-CELL-O0EQ-LOW-ENDPOINT",
      equilibrium_id = "N3V3-EQ-O0EQ-LOW-ENDPOINT",
      branch = "low",
    ...
    list(
      cell_id = "N3V3-CELL-O0EQ-EXCLUDE",
      equilibrium_id = "N3V3-EQ-O0EQ-EXCLUDE",
      branch = "exclude",
      conditions = c("o_0=1/m<o_1", "0<nu<=1"),
      classification = "R1 passage without H at y=0"
    ),
    list(
      cell_id = "N3V3-CELL-O1EQ-LOW",
      equilibrium_id = "N3V3-EQ-O1EQ-LOW",
      branch = "low",
    ...
    list(
      cell_id = "N3V3-CELL-O1EQ-EXCLUDE",
      equilibrium_id = "N3V3-EQ-O1EQ-EXCLUDE",
      branch = "exclude",
    ...
    list(
      cell_id = "N3V3-CELL-O1EQ-POOL",
      equilibrium_id = "N3V3-EQ-O1EQ-POOL",
      branch = "pool",
    ...
    list(
      cell_id = "N3V3-CELL-O1EQ-MIXED-EP",
      equilibrium_id = "N3V3-EQ-O1EQ-MIXED-EP",
      branch = "mixed_ep",
      conditions = c("o_0<o_1=1/m", paste0(nu_se, "<nu<=1"), paste0(h_e, "=", h_p)),
  ...
  claims <- list(
    list("N3V3-C01", "continuation", "N1-EQ-01 supplies 1/m to each weak state and o_theta to H in R2 current units; beta is applied exactly once in R1."),
    list("N3V3-C02", "weak ballot", "A weak nonproposer votes yes if and only if x_j>=beta/m, with T^Y selecting yes at equality."),
    list("N3V3-C03", "H ballot", "H's complete best-response map covers nonpivotal passage, pivotal passage, and inevitable failure after every feasible proposal."),
    list("N3V3-C04", "proposer deviations", "The closed proposer-payoff map covers every feasible proposal and both types under the true prior."),
    list("N3V3-C05", "candidate reduction", "Every payoff-maximizing pure proposal reduces to exclusion, low-type-only, or pooling; deliberate failure remains in the deviation set."),
    list("N3V3-C06", "P0", "Every selected proposal exhausts the pie because increasing the proposer residual preserves the belief-invariant response and outcome maps induced by frozen N1; no on-path slack survives."),
    list("N3V3-C07", "P1 and P1a", "Every pass-without-H proposal with y>0 is strictly dominated by shifting y to r_i, so every selected exclusion proposal has y=0."),
    list("N3V3-C08", "delay", "Deliberate failure pays beta/m and is strictly dominated by exclusion by 1-beta*q/m>0; screening delay survives with probability nu in selected low-type-only cells."),
    list("N3V3-C09", "feasibility", "Every selected low-type-only, pooling, and exclusion candidate is strictly feasible in its declared cell."),
    list("N3V3-C10", "partition", "The eleven cells are mutually exclusive and exhaustive over all admissible primitives and nu, including every equality and endpoint."),
    list("N3V3-C11", "o_1=1/m", "The residual E=P proposer-payoff tie is resolved by the expected-H comparison, with both branches and every mixture retained at exact equality."),
    list("N3V3-C12", "multiplicity", "All coalition identities, pure proposer-identity assignments, and proposer mixtures are parameterized without a symmetry restriction."),
    list("N3V3-C13", "beliefs", "Positive-probability proposals obey Bayes, zero-prior type strategies remain specified, and every zero-probability proposal-vote vector has an explicit unrestricted posterior at every nu."),
    list("N3V3-C14", "P7", "The public H vote enters every positive-probability posterior update; weak votes are type-independent."),
    list("N3V3-C15", "weak payoff map", "Each record exports the complete identity-indexed pre-recognition weak payoff map."),
    list("N3V3-C16", "transport sufficiency", "Every symbol in admissibility_conditions, selection_status, hegemon_payoff_by_type, and outcome_distribution is primitive or defined inside that transported record."),
    list("N3V3-C17", "existence", "An equilibrium exists throughout the declared domain; there is no nonexistence cell under beta in (0,1).")
  )

  ledger <- list(
    schema_version = "essential-input-claim-ledger-v3",
    node = "N3",
    ...
    claims = lapply(claims, function(entry) {
      list(
        ...
        status = "proved",
        evidence = paste0(
          "model_redesign/essential_input_n3_r1_majority_derivation_v3.md#claim-",
          tolower(entry[[1L]])
        )
      )
    })
...
direct_execution <- function() {
  ...
  length(script_arguments) == 1L &&
    identical(basename(sub("^--file=", "", script_arguments)), "build_essential_input_n3_v3.R")
}
...
main <- function() {
  ...
  default_ledger_path <- file.path(
    repository_root,
    "model_redesign",
    "essential_input_n3_claim_ledger_v3.json"
  )
  ...
  assert_true(
    ...
    "N3 v3 may consume only the authorized frozen N1 bytes."
  )

  objects <- make_n3_v3_objects(n1_hash)
  ...
  if (!quiet) {
    ...
    cat("N3V3-SHA-256:", sha256_file(interface_path), "\n")
