...
# Standalone N3 v4 builder. It imports no N3 v2/v3 object or function; the
...
make_n3_v4_objects <- function(n1_hash) {
  common_domain <- c(
...
direct_execution <- function() {
  ...
  length(script_arguments) == 1L &&
    identical(basename(sub("^--file=", "", script_arguments)), "build_essential_input_n3_v4.R")
}
...
main <- function() {
  ...
  assert_true(
    ...
    "N3 v4 may consume only the authorized frozen N1 bytes."
  )

  objects <- make_n3_v4_objects(n1_hash)
  ...
  if (!quiet) {
    ...
    cat("N3V4-SHA-256:", sha256_file(interface_path), "\n")
