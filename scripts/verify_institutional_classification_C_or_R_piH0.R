...
# Conditional institutional classification check under the pi_H = 0 C-or-R
# baseline. This excludes the high-state-only feasibility branch; use
...
compute_classification_C_or_R <- function(N, r, alpha, beta, costs) {
  ...
  V_W_reject <- function(mu) beta * g(mu) / m
  V_W_U <- function(mu) {
    ifelse(pool_feasible(mu) & pool_preferred(mu), V_W_pool(mu), V_W_reject(mu))
  }
...
objects <- do.call(compute_classification_C_or_R, params)

cat("C-or-R institutional classification check under pi_H = 0\n")
...
cat("\nInstitutional classification C-or-R check passed for the OPEC calibration.\n")
