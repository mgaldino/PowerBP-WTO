...
assert_true(
  ...
  "N3 v2 may consume only the authorized frozen N1 bytes."
)
...
branch_weight_condition <- function(branch) {
  ...
  stop("Unknown branch.", call. = FALSE)
}
...
common_strategy <- function(branch) {
  list(
    ...
    feasibility = switch(
      ...
      exclude = paste0(
        "Every exclusion proposal is strictly feasible because y=0<=y_bar and beta*(q-1)/m<1."
      ),
      mixed_ep = paste0(
...
belief_system <- function(branch) {
  continuation <- if (identical(branch, "low")) {
    paste0(
      "After a positive-probability R1 failure, H voted no under theta=1 while theta=0 voted yes, ",
      "so Bayes gives the R2 posterior 1. If nu=0, every failure history is zero probability and ",
      "its posterior is an arbitrary eta_i(s,v) in [0,1]."
    )
  } else {
    paste0(
      "The selected proposal passes in R1. Every failure vector therefore has zero probability ",
      "and receives an arbitrary eta_i(s,v) in [0,1]."
    )
  }
  list(
    entry = "Pr(theta=1)=nu.",
    ...
    published_vote_vector = continuation,
...
hegemon_payoff <- function(branch) {
  if (identical(branch, "low")) {
    return(list(theta_0 = "beta*o_0", theta_1 = "beta*o_1"))
  }
  if (identical(branch, "pool")) {
    return(list(theta_0 = "beta*o_1", theta_1 = "beta*o_1"))
  }
  if (identical(branch, "exclude")) {
    return(list(theta_0 = "o_0", theta_1 = "o_1"))
  }
  list(
    theta_0 = paste0(
...
selection_status <- function(branch) {
  ...
  paste0(
    ...
    "Arrays may differ across proposer identities. Because proposer payoff and expected H payoff tie exactly in this cell, all pure identity assignments and all proposer mixtures are retained, with no symmetry or further selection."
  )
}
...
make_record <- function(region) {
  ...
  list(
    ...
    checks_performed = c(
      "N3V2-C01 frozen N1 import and exactly-one discount",
      "N3V2-C02 weak voting cutoff and T^Y",
      "N3V2-C03 complete H best-response map",
      "N3V2-C04 proposer payoff map for every feasible proposal",
      "N3V2-C05 exhaustive reduction to exclusion, low-type-only, pooling, and deliberate failure",
      "N3V2-C06 P0 full-pie use by every selected proposal",
      "N3V2-C07 P1 strict hedge dominance and P1a",
      "N3V2-C08 deliberate failure strictly dominated because 1-beta*q/m>0",
      "N3V2-C09 candidate feasibility in every selected cell",
      "N3V2-C10 strict-region and equality-boundary partition",
      "N3V2-C11 o_1=1/m residual proposer tie and expected-H tie-break",
      "N3V2-C12 pure identity assignments and all proposer mixtures",
      "N3V2-C13 on-path Bayes, zero-prior types, and unrestricted off-path beliefs",
      "N3V2-C14 P7 public H vote and posterior update",
      "N3V2-C15 identity-indexed weak payoff map",
      "N3V2-C16 closed N6-transported fields without free symbols",
      "N3V2-C17 complete existence and endpoint coverage"
    ),
...
h_exclusion_o1_equal <- "(1-nu)*o_0+nu/m"
h_pooling_o1_equal <- "beta/m"

regions <- list(
  list(
    cell_id = "N3V2-CELL-O1LT-LOW",
    equilibrium_id = "N3V2-EQ-O1LT-LOW",
    branch = "low",
  ...
  list(
    cell_id = "N3V2-CELL-O1LT-POOL",
    equilibrium_id = "N3V2-EQ-O1LT-POOL",
    branch = "pool",
    conditions = c("o_1<1/m", paste0(nu_sp, "<nu<=1")),
    classification = "pooling R1 passage with H"
  ),
  list(
    cell_id = "N3V2-CELL-CROSS-LOW",
    equilibrium_id = "N3V2-EQ-CROSS-LOW",
    branch = "low",
  ...
  list(
    cell_id = "N3V2-CELL-CROSS-EXCLUDE",
    equilibrium_id = "N3V2-EQ-CROSS-EXCLUDE",
    branch = "exclude",
    conditions = c("o_0<1/m<o_1", paste0(nu_se, "<nu<=1")),
    classification = "R1 passage without H at y=0"
  ),
  list(
    cell_id = "N3V2-CELL-O0GT-EXCLUDE",
    equilibrium_id = "N3V2-EQ-O0GT-EXCLUDE",
    branch = "exclude",
    conditions = c("1/m<o_0<o_1", "0<=nu<=1"),
    classification = "R1 passage without H at y=0"
  ),
  list(
    cell_id = "N3V2-CELL-O0EQ-LOW-ENDPOINT",
    equilibrium_id = "N3V2-EQ-O0EQ-LOW-ENDPOINT",
    branch = "low",
  ...
  list(
    cell_id = "N3V2-CELL-O0EQ-EXCLUDE",
    equilibrium_id = "N3V2-EQ-O0EQ-EXCLUDE",
    branch = "exclude",
    conditions = c("o_0=1/m<o_1", "0<nu<=1"),
    classification = "R1 passage without H at y=0"
  ),
  list(
    cell_id = "N3V2-CELL-O1EQ-LOW",
    equilibrium_id = "N3V2-EQ-O1EQ-LOW",
    branch = "low",
  ...
  list(
    cell_id = "N3V2-CELL-O1EQ-EXCLUDE",
    equilibrium_id = "N3V2-EQ-O1EQ-EXCLUDE",
    branch = "exclude",
    conditions = c(
      "o_0<o_1=1/m",
      paste0(nu_se, "<nu<=1"),
      paste0(h_exclusion_o1_equal, "<", h_pooling_o1_equal)
    ),
    classification = "measure-zero residual E=P proposer-payoff tie; exclusion uniquely minimizes expected H payoff"
  ),
  list(
    cell_id = "N3V2-CELL-O1EQ-POOL",
    equilibrium_id = "N3V2-EQ-O1EQ-POOL",
    branch = "pool",
    conditions = c(
      "o_0<o_1=1/m",
      paste0(nu_se, "<nu<=1"),
      paste0(h_pooling_o1_equal, "<", h_exclusion_o1_equal)
    ),
    classification = "measure-zero residual E=P proposer-payoff tie; pooling uniquely minimizes expected H payoff"
  ),
  list(
    cell_id = "N3V2-CELL-O1EQ-MIXED-EP",
    equilibrium_id = "N3V2-EQ-O1EQ-MIXED-EP",
    branch = "mixed_ep",
    conditions = c(
      "o_0<o_1=1/m",
      paste0(nu_se, "<nu<=1"),
...
equilibrium_ids <- vapply(regions, `[[`, character(1), "equilibrium_id")
claims <- list(
  list("N3V2-C01", "continuation", "N1-EQ-01 supplies 1/m to each weak state and o_theta to H in R2 current units; beta is applied exactly once in R1."),
  list("N3V2-C02", "weak ballot", "A weak nonproposer votes yes if and only if x_j>=beta/m, with T^Y selecting yes at equality."),
  list("N3V2-C03", "H ballot", "H's complete best-response map covers nonpivotal passage, pivotal passage, and inevitable failure after every feasible proposal."),
  list("N3V2-C04", "proposer deviations", "The closed proposer-payoff map covers every feasible proposal and both types under the true prior."),
  list("N3V2-C05", "candidate reduction", "Every payoff-maximizing pure proposal reduces to exclusion, low-type-only, or pooling; deliberate failure remains in the deviation set."),
  list("N3V2-C06", "P0", "Every selected proposal exhausts the pie; no on-path slack survives."),
  list("N3V2-C07", "P1 and P1a", "Every pass-without-H proposal with y>0 is strictly dominated by shifting y to r_i, so every selected exclusion proposal has y=0."),
  list("N3V2-C08", "delay", "Deliberate failure pays beta/m and is strictly dominated by exclusion by 1-beta*q/m>0; screening delay survives with probability nu in selected low-type-only cells."),
  list("N3V2-C09", "feasibility", "Every selected low-type-only, pooling, and exclusion candidate is strictly feasible in its declared cell."),
  list("N3V2-C10", "partition", "The eleven cells are mutually exclusive and exhaustive over all admissible primitives and nu, including every equality and endpoint."),
  list("N3V2-C11", "o_1=1/m", "The residual E=P proposer-payoff tie is resolved by the expected-H comparison, with both branches and every mixture retained at exact equality."),
  list("N3V2-C12", "multiplicity", "All coalition identities, pure proposer-identity assignments, and proposer mixtures are parameterized without a symmetry restriction."),
  list("N3V2-C13", "beliefs", "Positive-probability proposals obey Bayes, zero-prior type strategies remain specified, and every zero-probability proposal or vote history has an explicit unrestricted belief."),
  list("N3V2-C14", "P7", "The public H vote enters every positive-probability posterior update; weak votes are type-independent."),
  list("N3V2-C15", "weak payoff map", "Each record exports the complete identity-indexed pre-recognition weak payoff map."),
  list("N3V2-C16", "transport sufficiency", "Every symbol in admissibility_conditions, selection_status, hegemon_payoff_by_type, and outcome_distribution is primitive or defined inside that transported record."),
  list("N3V2-C17", "existence", "An equilibrium exists throughout the declared domain; there is no nonexistence cell under beta in (0,1).")
)

ledger <- list(
  schema_version = "essential-input-claim-ledger-v2",
  node = "N3",
  candidate_status = "pending_independent_review",
  source_interface = list(
    record_id = "N1-EQ-01",
    artifact_hash = n1_hash
  ),
...
write_canonical_json(interface, interface_path)
write_canonical_json(ledger, ledger_path)

cat("BUILT:", sub(paste0(repository_root, "/"), "", interface_path, fixed = TRUE), "\n")
cat("BUILT:", sub(paste0(repository_root, "/"), "", ledger_path, fixed = TRUE), "\n")
cat("N1-SHA-256:", n1_hash_bare, "\n")
cat("N3V2-SHA-256:", sha256_file(interface_path), "\n")
