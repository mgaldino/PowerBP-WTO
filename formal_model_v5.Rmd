...
The dominant explanations for consensus in international organizations fall into two categories, each missing a different piece of the puzzle. Self-binding accounts hold that powerful states accept formal constraints to make credible commitments, encouraging weaker partners to participate [@keohane1984after; @ikenberry2001after]. These accounts explain why consensus sustains cooperation, but not why it confers a distributive advantage on the hegemon. Informal power accounts imply that agenda power, control over drafting, and asymmetric resources enable powerful states to shape outcomes regardless of formal rules [@steinberg2002shadow; @stone2011controlling; @gruber2000ruling]. They cannot explain why formal rules vary systematically across organizations. @gould2016consensus documents the prevalence and variation of consensus comprehensively. This paper identifies a mechanism absent from existing accounts: consensus activates informational power. The hegemon’s private information is more valuable under unanimity than under majority rule.

The model builds on Baron-Ferejohn's legislative bargaining, where proposal rights drive political power [@baron1989bargaining; @kalandrakis2006proposal; @eraslan2019legislative]. Holding proposal rights symmetric isolates the effect of the voting rule. The departure is combining asymmetric information about the value of cooperation with a choice between voting rules. Under unanimity, the combination generates endogenous screening; under majority, it does not.

The most closely related papers to the present one are @koremenos2001rational and @kim2025persuasion. The paper by @kim2025persuasion examines persuasion in veto bargaining, analyzing how an information designer augments a proposer's agenda-setting power when a veto player is present. Their model is bilateral and takes the veto rule as given. The article by @koremenos2001rational conjectures (though it does not formalize them) that powerful states seek asymmetric control, while uncertainty increases demand for unanimity as insurance for risk-averse states. The present model shows that uncertainty indeed makes the hegemon’s private information valuable, and there is no need to appeal to risk aversion^[For information design under unanimity without bargaining, see @bardhi2018modes.].

The informational mechanism generates a discriminating prediction. Because screening rents require genuine uncertainty about the value of cooperation, the model predicts a negative cross-sectional association between consensus rules and transparent distributive stakes. Financial institutions, where capital contributions and loan terms make the pie calculable, should not adopt the consensus rule and instead use weighted majority. Regulatory and standard-setting bodies, where consequences are difficult to forecast, should adopt consensus most frequently [@gould2016consensus]. Rational design accounts also predict unanimity under uncertainty [@koremenos2001rational], but through risk aversion rather than informational power. The two channels differ in their distributional implications: insurance protects all members symmetrically, while screening benefits the privately informed. Legitimacy accounts do not condition on informational structure. Self-binding accounts predict consensus where commitment problems are severe, regardless of transparency. Informal power accounts cannot explain why consensus is absent precisely where powerful states have the strongest informal leverage.
...
The game proceeds in three stages. In Stage 0, the hegemon chooses the voting rule $R \in \{M, U\}$. In Stage 1, nature draws $\theta$ (observed by $H$ only); weak states simultaneously decide whether to enter, paying cost $c$. The institution forms if and only if all $N-1$ weak states enter.\footnote{The all-or-nothing entry assumption and the interpretation of $N$ are discussed in the Scope section.} In Stage 2, members play a two-round Baron-Ferejohn bargaining game:\footnote{Two rounds ensure that rejection leads to a continuation game rather than immediate disagreement, so the screening mechanism does not depend on ultimatum structure. The motivating example (Section \@ref(example)) shows it operates in one round; two rounds confirm robustness in the Baron-Ferejohn environment.} in each round, a proposer is selected uniformly at random, offers a division of $V(\theta)$, and all members vote under rule $R$. Rejection in Round 1 leads to Round 2 with payoffs discounted by $\beta$; rejection in Round 2 yields disagreement payoffs.

The solution concept is Perfect Bayesian Equilibrium (PBE).\footnote{Tie-breaking convention: when $H$ is indifferent between accepting and rejecting an offer (offer equals reservation value), $H$ accepts.} Each weak state's information sets pool the two branches of $\theta$: at any decision node, $W_i$ knows the history of proposals and responses, but not $\theta$. Weak states hold belief $\mu$ over $\theta$, which equals the prior $p$ at the entry stage and may be updated during bargaining (e.g., after a rejection in Round 1). This asymmetry is what generates the screening problem when a weak state proposes under unanimity.

Figure \ref{fig:gametree-b} presents the extensive form of the bargaining subgame under unanimity (Stage 2).
...
The binary type space is also less restrictive than it first appears. Binary types provide the cleanest way to show how pivotality creates screening. Appendix C shows that the screening rent under unanimity is strictly positive for any continuous distribution with full support, while majority eliminates it entirely. For the uniform distribution, the dominance threshold satisfies $\alpha^*_{\text{cont}} \geq \alpha^*$, suggesting that binary types are the most conservative specification for the mechanism rather than an optimistic modeling choice.

The mechanism is most relevant when four conditions hold: informational asymmetry is substantively important ($H$ knows $\theta$, while weak states do not); the hegemon has a valuable outside option ($\alpha > 0$); entry is costly and belief-dependent; and the voting rule makes the hegemon pivotal, either formally or through strategic inclusion.

Figure \@ref(fig:heatmap-alpha-mu) maps the conditional dominance region in the $(\alpha, \mu)$ plane. Blue regions mark beliefs at which unanimity gives the hegemon a higher continuation payoff; red regions mark beliefs at which majority dominates. The solid curve traces the frontier $\bar\mu(\alpha)$ from Remark \ref{rem:mu_bar}. Two patterns stand out. First, the frontier is nearly flat: increasing $\alpha$ well beyond $\alpha^*$ shifts $\bar\mu$ only modestly downward, confirming that the screening advantage persists across most of the belief space. Second, the red region clusters near $\mu = 1$, where weak states are nearly certain about the gains from cooperation. This is precisely where informational asymmetry has the least value, and the cost of buying unanimous agreement dominates.


```{r heatmap-alpha-mu, echo=FALSE, fig.width=10, fig.height=7, fig.pos="H", fig.cap="Conditional payoff dominance in the $(\\alpha, \\mu)$ plane. Blue: unanimity dominates ($D(\\mu) > 0$). Red: majority dominates ($D(\\mu) < 0$). Solid curve: belief threshold $\\bar\\mu(\\alpha)$ above which majority dominates (Remark~\\ref{rem:mu_bar}). Dashed line: $\\alpha^*$. Left column: $r = 1.5$. Right column: $r = 2$. Top row: $N = 30$. Bottom row: $N = 164$. All panels: $\\beta = 0.9$."}
...
Finally, the scope condition $\alpha < \alpha^*$ should be read as the region in which majority can never dominate through a higher conditional-on-entry payoff. In that region, any advantage of majority must operate through entry: majority may sustain participation at priors for which unanimity does not, because $\tau(U) > \tau(M)$. This is most likely when entry costs are high, when $\alpha$ approaches $\alpha^*$, or when information is nearly public.

The boundary is sharp. At $\alpha = \alpha^*$, the payoff difference $D(\mu)$ equals zero exactly at $\mu = 1$. Because $\alpha^*$ decreases with $N$, the condition is more demanding in large organizations, where unanimity requires buying many more votes than majority. For $\alpha > \alpha^*$, majority can dominate near $\mu = 1$, where weak states nearly know the hegemon's type and the cost of buying unanimous agreement outweighs the screening advantage. Away from that near-certainty region, unanimity can still dominate. Figure \@ref(fig:heatmap-alpha-mu) illustrates this pattern: even in the $N=164$ panels, the blue region covers most of the $(\alpha,\mu)$ plane, while the red region clusters near $\mu=1$.
...
## B.5 Pending: general conditional dominance theorem {-}

\paragraph{Status.} Pending. The old proof of Theorem~\ref{thm:conditional} used two objects that are no longer valid globally under strict BF:

1. the old majority coefficient with the factor $(1-\alpha)$ in weak-state continuation values;
2. the old single-cutoff R1 unanimity formula.

The general theorem must be rederived either as a set of sufficient conditions based on the corrected piecewise regimes, or as a theorem based on the selection-free lower bound in Appendix B.3. Until that derivation is completed, Theorem~\ref{thm:conditional} should not be treated as proven in its current global form.

## B.6 Pending: formation-set nesting {-}

\paragraph{Status.} Pending. The old proof of $\mathcal F_U\subseteq\mathcal F_M$ used a budget-balance identity under majority. With external outside options, majority payoffs include $H$'s outside-option payoff even when $H$ is excluded, so that budget-balance step is invalid. The corrected proof must compare weak-state payoffs directly, using the corrected $\kappa_M^E$ and the rederived unanimity weak-state payoff.
...
## B.8 Pending: institutional classification {-}

\paragraph{Status.} Pending. The old classification requires three ingredients that are not yet all proven under the corrected model:

1. conditional dominance of unanimity on the relevant formation set;
2. $\mathcal F_U\subseteq\mathcal F_M$;
3. $\lambda_M^E>\alpha$ for the case in which majority forms and unanimity does not.

The third condition is equivalent to $\alpha<1-\beta(q-1)/N$ and must be stated explicitly if that case is retained.
...
\paragraph{Status.} Pending rederivation. The continuous-type appendix below was written under the superseded majority accounting convention and before the strict BF feasibility correction. It is retained in source as a draft, but it is hidden from the compiled appendix and should not be cited as a proven extension until majority payoffs, W-proposer feasibility, and H-proposer continuation values are rederived under the corrected model.
