...
```{r fig-terminal-regions, fig.cap="Terminal unanimity regions. The boundary is \\(\\mu_2^*=(t_1-t_0)/(1-t_0)\\), holding \\(t_0=0.19\\) fixed. The point marks the working numerical illustration.", out.width='95%'}
...
Figure \ref{fig:fig-r1-regions} plots the geometry of the maximum operator in Proposition \ref{prop:r1}. It uses a strict dynamic-gap slice, \(a_1-a_0(1)=0.04\), to display all three candidates. The working numerical illustration below lies on the boundary \(a_1=a_0(1)\), so strict low-only separation is blocked there. Appendix Table \ref{tab:r1-diagnostic-examples} reports a nonboundary diagnostic case in which low-only and pooling are both selected on nonempty belief regions; that example is illustrative and is not an empirical calibration.
...
This figure is the main nonboundary diagnostic for the Round-1 geometry: it uses a strict \(a_0(1)<a_1\) slice, unlike the working numerical illustration in Example \ref{ex:calibration}. Appendix Table \ref{tab:r1-diagnostic-examples} gives the corresponding numerical ranges for the same candidate logic.

The nonboundary case can also be seen in a simple numerical diagnostic. Keeping \(N=13\), set \(\beta=0.6\), \(t_0=0.02\), \(t_1=0.27\), \(o_0=0.25\), and \(o_1=0.27\). Then \(a_0(1)=0.07<a_1=0.162\), so strict low-only separation is admissible. Evaluating the pairwise inequalities in (13a)--(13c) on a fine belief grid, the selected R1 path tests the low type at low beliefs and pools at higher beliefs:

```{r r1-nonboundary-main}
r1_examples_main <- read.csv("tables/relative_package_r1_examples_piH0.csv")
r1_low_only_main <- subset(r1_examples_main, example == "low_only_example")
r1_low_only_fmt <- data.frame(
  Candidate = candidate_label(r1_low_only_main$candidate),
  `mu range` = fmt_range(r1_low_only_main$mu_min, r1_low_only_main$mu_max, 3),
  `a0(1)` = fmt_num(r1_low_only_main$a0_post1, 3),
  `a1` = fmt_num(r1_low_only_main$a1, 3),
  check.names = FALSE
)
knitr::kable(
  r1_low_only_fmt,
  booktabs = TRUE,
  caption = "Non-calibrated nonboundary diagnostic for Proposition \\ref{prop:r1}. The belief ranges are grid-derived from the candidate inequalities in (13a)--(13c) and show that strict low-only and pooling regions are both live under the maintained assessment."
)
```
...
# Working Numerical Illustration {#calibration}

\begin{example}[Working numerical illustration]\label{ex:calibration}
...
\[
\mu_2^*\approx0.117,\qquad
a_1\approx0.257,\qquad
a_0(1)\approx0.257,
\]
so High-Posterior Pooling holds and the illustration lies on the R1 Dynamic Threshold Order boundary \(a_0(1)=a_1\). Strict low-only separation is therefore blocked in this numerical illustration, and the selected Round-1 unanimity candidate is pooling for all beliefs. Majority satisfies No-Cheap-H because
\[
a_0^M=0.171>\frac{\beta}{m}=0.075.
\]
The weak-state entry payoffs are
\[
V_W^U(\mu)=\frac{1-a_1}{12}\approx0.062,
\qquad
V_W^M(\mu)=\frac{1}{12}\approx0.083.
\]
The hegemon's payoff gap is
\[
\Delta_H(\mu)\approx0.067-0.095\mu.
\tag{32}
\]
Thus, when both rules form, \(H\) prefers unanimity for \(\mu<0.7\), is indifferent at \(\mu=0.7\), and prefers majority for \(\mu>0.7\).
\end{example}

```{r fig-deltah, fig.cap="Conditional payoff gap for the hegemon in the working numerical illustration. The dashed vertical line marks \\(\\Delta_H(\\mu)=0\\) at \\(\\mu=0.7\\).", out.width='90%'}
...
\midrule
Strict No-Cheap-H & \(a_0^M=0.171\) & \(\beta/m=0.075\) & \(0.096\) \\
\(t_0\geq0\) & \(0.19\) & \(0\) & \(0.19\) \\
\(t_1\geq t_0\) & \(0.285\) & \(0.19\) & \(0.095\) \\
\(\bar y\geq t_1\) & \(1\) & \(0.285\) & \(0.715\) \\
\(1\geq\bar y\) & \(1\) & \(1\) & \(0\) \\
\bottomrule
\end{tabular}
\caption{Numerical-illustration margins for Strict No-Cheap-H and the terminal Threshold Order. The zero slack in the last row is the normalization \(\bar y=1\).}
...
\midrule
\(P\) feasibility at max \(c(\mu)\) & \(\approx0.925\) & \(1\) & \(\approx0.075\) \\
\(D\) feasibility & \(0\) & \(1\) & \(1\) \\
\(a_1-a_0(1)\) & \(0\) & \(>0\) for strict \(L\) & Boundary; strict \(L\) blocked \\
\(\Pi_P^U(0)-\Pi_D^U(0)\) & \(\approx0.015\) & \(0\) & \(\approx0.015\) \\
\(\Pi_P^U(\mu_2^*)-\Pi_D^U(\mu_2^*)\) & \(\approx0.1\) & \(0\) & \(\approx0.1\) \\
\(\Pi_P^U(0.5)-\Pi_D^U(0.5)\) & \(\approx0.1\) & \(0\) & \(\approx0.1\) \\
\(\Pi_P^U(1)-\Pi_D^U(1)\) & \(\approx0.1\) & \(0\) & \(\approx0.1\) \\
\bottomrule
\end{tabular}
\caption{Round-1 numerical-illustration margins under the weak-vote-passive assessment. The equality \(a_0(1)=a_1\) places the low-only candidate on the strict-separation boundary, while pooling beats no-information delay at the displayed beliefs.}
...
\midrule
\(V_W^M\) & \(\approx0.083\) & Representative weak payoff under majority \\
\(V_W^U\) & \(\approx0.062\) & Representative weak payoff under selected unanimity path \\
\(V_W^M-V_W^U\) & \(\approx0.021\) & Entry/nesting gap \\
\bottomrule
\end{tabular}
\caption{Entry and nesting margin in the working numerical illustration.}
...
\bottomrule
\end{tabular}
\caption{Hegemon payoff-gap margins in the working numerical illustration. Positive values favor unanimity; negative values favor majority.}
...
\midrule
\(\chi\leq(1-a_1)/12\approx0.06196\) & Both rules form; \(H\) prefers unanimity for \(\mu<0.7\) \\
\((1-a_1)/12<\chi\leq1/12\approx0.08333\) & Only majority forms \\
\(\chi>1/12\approx0.08333\) & No rule forms \\
\bottomrule
\end{tabular}
\caption{Illustrative institutional classification under the fixed-pie relative-package baseline. At \(\mu=0.7\), \(H\) is indifferent when both rules form; for \(\mu>0.7\), \(H\) prefers majority.}
\label{tab:calibration}
\end{table}

```{r fig-classification, fig.cap="Illustrative entry and institutional classification. The horizontal boundaries are \\(V_W^U=(1-a_1)/12\\approx0.06196\\) and \\(V_W^M=1/12\\approx0.08333\\); the dashed vertical line is \\(\\Delta_H(\\mu)=0\\).", out.width='95%'}
knitr::include_graphics("figures/relative_package_classification_piH0.pdf")
```

The equality \(a_0(1)=a_1\) in the working numerical illustration should not be hidden. It is a boundary case for strict low-only separation. Table \ref{tab:robustness-windows} therefore reports one-way perturbation windows, not an open neighborhood around that exact parameter vector. The "core classification" window keeps the maintained threshold domains, Strict No-Cheap-H, entry nesting, and an interior zero of \(\Delta_H(\mu)\). The "pooling-only" window is a different one-way diagnostic, not a subset of the core window: it records when the selected R1 candidate remains pooling for every belief. Degenerate pooling-only rows mean that the all-belief pooling selection is a boundary feature of the illustration, not the robust object.

```{r robustness-windows}
robustness_windows <- read.csv("tables/relative_package_robustness_windows_piH0.csv")
robustness_windows_fmt <- data.frame(
  Parameter = robustness_windows$parameter,
  Baseline = fmt_num(robustness_windows$baseline, 3),
  `Core classification window` = fmt_range(
    robustness_windows$core_min,
    robustness_windows$core_max,
    3
  ),
  `Pooling-only window` = fmt_range(
    robustness_windows$pooling_all_min,
    robustness_windows$pooling_all_max,
    3
  ),
  check.names = FALSE
)
knitr::kable(
  robustness_windows_fmt,
  booktabs = TRUE,
  position = "H",
  caption = "One-way perturbation windows from the working numerical illustration. The row \\(gap\\) varies \\(t_1-t_0\\) by holding \\(t_0=0.19\\) fixed and setting \\(t_1=t_0+gap\\)."
)
```

Tables \ref{tab:classification-sweeps-r1} and \ref{tab:classification-sweeps-entry} give representative parameter perturbations. These checks are local diagnostics, not a proof of an open robustness neighborhood around the exact numerical illustration. They show that entry can remain nested and the payoff gap can retain an interior crossing after substantively meaningful one-way perturbations. The selected R1 path can change, however, so the paper should not sell "pooling for all beliefs" as the robust object. The robust object is the stated-domain institutional result.

```{r classification-sweeps-r1}
classification_sweeps <- read.csv("tables/relative_package_classification_sweeps_piH0.csv")
classification_sweeps_r1_fmt <- data.frame(
  Case = classification_sweeps$case,
  `beta` = fmt_num(classification_sweeps$beta, 2),
  `t1` = fmt_num(classification_sweeps$t1, 3),
  `o1` = fmt_num(classification_sweeps$o1, 3),
  `R1 candidates` = candidate_label(classification_sweeps$selected_candidates),
  `switch mu` = classification_sweeps$candidate_switches,
  `Delta_H=0` = classification_sweeps$Delta_H_roots,
  check.names = FALSE
)
knitr::kable(
  classification_sweeps_r1_fmt,
  booktabs = TRUE,
  position = "H",
  caption = "Representative R1-path robustness checks for the illustrative classification. Candidate-switch beliefs and \\(\\Delta_H=0\\) roots are computed separately. All rows satisfy the maintained domains and have an interior \\(\\Delta_H=0\\) crossing."
)
```

```{r classification-sweeps-entry}
classification_sweeps_entry_fmt <- data.frame(
  Case = classification_sweeps$case,
  `max V_W^U` = fmt_num(classification_sweeps$W_U_max, 3),
  `V_W^M` = fmt_num(classification_sweeps$W_M, 3),
  `min M-U gap` = fmt_num(classification_sweeps$min_entry_gap, 3),
  check.names = FALSE
)
knitr::kable(
  classification_sweeps_entry_fmt,
  booktabs = TRUE,
  position = "H",
  caption = "Weak-state entry margins for the representative robustness checks. Positive M-U gaps verify entry nesting in each displayed case."
)
```
...
The working numerical illustration should be read in this spirit. The parameters \(t_0=0.19\) and \(t_1=0.285\) are not direct estimates of Saudi payoffs. They describe a threshold gap large enough to make pivotal approval informationally valuable, while retaining a fixed weak surplus and no formal proposal advantage for \(H\). In this illustration, unanimity forms only when \(\chi\leq(1-a_1)/12\approx0.06196\), majority forms up to \(1/12\approx0.08333\), and when both form the pivotal actor prefers unanimity for \(\mu<0.7\). The classification therefore replaces a simple "consensus always helps the hegemon" claim with a more precise prediction: consensus helps the hegemon only where the pivotality-screening rent exceeds the payoff from being excluded under majority.
...
\(\mathcal C_U^H(\chi)\): both form, \(H\) prefers unanimity & Both institutions are viable, and consensus gives Saudi Arabia a pivotality rent large enough to beat its payoff when excluded under majority. \\
\(\mathcal C_T^H(\chi)\): both form, tie & Both institutions are viable, and the pivotality rent exactly offsets the majority outside payoff. \\
\(\mathcal C_M^H(\chi)\): both form, \(H\) prefers majority & Both institutions are viable, but beliefs make the private-threshold rent too small to compensate Saudi Arabia for the majority benchmark. \\
\bottomrule
\end{tabular}
...
The weak-vote-passive assessment is also a scope condition. It is natural here because weak states have no private information about \(H\)'s type. Their unexpected votes should not be interpreted as signals about information they do not possess. But this is a maintained equilibrium assessment, not a theorem about every unrestricted PBE. A different belief discipline could support additional off-path behavior, especially after histories that the baseline treats as uninformative. Table \ref{tab:assessment-scope} separates the assessment-free benchmark components from the assessment-dependent R1 selection. The paper's formal claim is therefore a clean institutional classification under the stated voting and belief protocol together with the threshold, approval, tie-break, No-Cheap-H, and fixed-pie collective-entry conditions used in the results.

Finally, the voting rule is treated as an institutional environment rather than an endogenous choice. This restriction is a disciplining simplification. If \(H\) selected the rule after observing its type, institutional choice could signal private information before bargaining begins. That is a real channel, but it would combine institutional signaling with the screening mechanism studied here. The discussion in Appendix B.3 records how signaling could reduce, leave unchanged, or amplify screening. The baseline holds that channel fixed to isolate informational power through pivotality. Continuous types, heterogeneous weak states, learning across repeated OPEC meetings, and positive recognition probability for \(H\) are also natural extensions. They should be added only after preserving the separation among outside-option payoffs, veto or pivotality power, and proposal power that the current architecture enforces.
...
The baseline also does not model endogenous constitutional rule choice. This is deliberate. If a privately informed hegemon chose unanimity or majority after observing its type, the choice of rule could itself become a signal. That signaling stage would interact with the screening mechanism studied here. It could reduce screening if rule choice reveals the type, leave screening intact if both types choose the same rule or if the rule is institutionally fixed, or amplify screening if rule choice shifts beliefs toward a high participation threshold without fully revealing the state. The baseline holds this channel fixed to isolate screening through pivotality.
...
The main text reports the most important comparative statics locally. This appendix collects the remaining ones in compact form.
...
\tag{B2}
\]
Strict low-only separation requires this slack to be positive. The working numerical illustration sets \(o_1-o_0=t_1-t_0\), so it lies exactly on the boundary. Perturbations that make \(o_1-o_0<t_1-t_0\) create a strict low-only interval; perturbations in the opposite direction violate the maintained threshold order and require a different case analysis.
...
Table \ref{tab:r1-diagnostic-examples} records three diagnostic cases: the working numerical illustration, a nonboundary case with a low-only region, and a delay case. The examples are not alternative empirical calibrations. They show that all three candidates in Proposition \ref{prop:r1} are live under the stated protocol.

```{r r1-diagnostic-examples}
r1_examples <- read.csv("tables/relative_package_r1_examples_piH0.csv")
r1_examples_fmt <- data.frame(
  Example = r1_examples$example,
  `beta` = fmt_num(r1_examples$beta, 2),
  `t0` = fmt_num(r1_examples$t0, 3),
  `t1` = fmt_num(r1_examples$t1, 3),
  `o0` = fmt_num(r1_examples$o0, 3),
  `o1` = fmt_num(r1_examples$o1, 3),
  `a0(1)` = fmt_num(r1_examples$a0_post1, 3),
  `a1` = fmt_num(r1_examples$a1, 3),
  Candidate = candidate_label(r1_examples$candidate),
  `mu range` = fmt_range(r1_examples$mu_min, r1_examples$mu_max, 3),
  check.names = FALSE
)
knitr::kable(
  r1_examples_fmt,
  booktabs = TRUE,
  caption = "Diagnostic Round-1 candidate regions under the weak-vote-passive assessment."
)
```
...
\]
For these parameters, strict low-only separation is blocked by \(a_0(1)=a_1=0.45\). Pooling is not selected at all beliefs: it becomes feasible and eventually beats delay once beliefs are high enough. The diagnostic table above reports delay on the low-belief interval and pooling above it. At \(\mu=0.01\), terminal bargaining is very attractive to a future recognized weak proposer because the high type is unlikely, but Round-1 unanimity requires compensating both \(H\) and all non-proposing weak voters. The pooling package is infeasible at this belief, and the proposer selects no-information delay.
