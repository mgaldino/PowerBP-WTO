...
Thus a weak-caused failed proposal cannot use \(H\)'s nonpivotal vote to sustain informative pure-strategy separation. The only incentive-compatible pure prescriptions relevant to the selected candidate comparison are pooling prescriptions for \(H\)'s nonpivotal ballot, and their on-path posterior is \(\mu\). Therefore weak-caused failures are payoff-equivalent to no-information delay \(D\) within the selected pure-strategy analysis. The argument does not restrict arbitrary off-path beliefs after deviations from pooling prescriptions and does not characterize all unrestricted PBEs. \(\square\)
