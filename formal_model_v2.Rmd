...
For $K = 3$ with values $(1, r_1, r_2)$, there exists a threshold $\alpha_3^*(N, \beta, r_1, r_2)$ such that for all $\alpha < \alpha_3^*$ and all $\mu \in \operatorname{int}(\Delta^2)$,
\[
D(\mu) \equiv E_\theta[V_H^{R1}(\theta, \mu, U)] - E_\theta[V_H^{R1}(\theta, \mu, M)] > 0.
\]
The threshold $\alpha_3^*$ is characterized numerically.
\end{proposition}

The proof strategy mirrors Lemma \ref{lem:conditional}: the payoff difference decomposes into a backbone term that is affine in $\mu$ (and positive under the parametric restriction on $\alpha$) plus correction terms at each screening boundary that are either non-negative or vanish at their activation point. With $K = 3$, there are four correction terms (two R2 boundaries and two R1 boundaries) rather than two, but the logic is unchanged.
...
The baseline parameterization ($r_1 = 1.5$, $r_2 = 2.5$, $\alpha = 0.3$, $\beta = 0.9$, $N = 5$) passes with $\min D = 0.031$ on a fine grid over $\Delta^2$. Failures occur at $\beta = 0.7$, consistent with the $K = 2$ model: $\alpha^*(5, 0.7) = 0.189 < 0.3$, so the parametric condition $\alpha < \alpha^*$ is violated. When the grid is re-run with $\alpha = 0.15$ (below $\alpha^*$), all parameterizations pass.
