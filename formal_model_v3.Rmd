...
With Bayesian persuasion, the hegemon's payoff under each rule is $\Pi_H^*(R, p) = \operatorname{cav} v(p, R)$. The institutional choice reduces to comparing these concavified values. The central result is that unanimity dominates majority for every prior at which the institution would form under unanimity.

\begin{theorem}[Dominance of unanimity]\label{thm:dominance}
Suppose $\alpha < \alpha^*(N, \beta)$. For every prior $p \geq \tau(U)$,
\[
\Pi_H^*(U, p) > \Pi_H^*(M, p).
\]
That is, whenever the institution can form under unanimity, the hegemon strictly prefers it.
\end{theorem}

\begin{proof}
Entry occurs under both rules since $\tau(U) \geq \tau(M)$. By Lemma \ref{lem:conditional}, $v(p, U) > v(p, M)$. Under majority, $v(\cdot, M)$ is affine above $\tau(M)$ and hence already concave, so $\operatorname{cav} v(p, M) = v(p, M)$. Therefore $\Pi_H^*(U, p) = \operatorname{cav} v(p, U) \geq v(p, U) > v(p, M) = \Pi_H^*(M, p)$.
\end{proof}

The theorem holds regardless of whether the entry set under unanimity is connected or disconnected, and regardless of the shape of the concavification. The screening mechanism (Proposition \ref{prop:jump}) gives the hegemon a higher conditional payoff, and majority's linearity means that concavification cannot close the gap. The only channel through which majority can dominate is the entry margin: priors so pessimistic that only majority can induce participation.

\begin{corollary}[Global dominance when entry is costless]\label{cor:global}
If $\tau(U) = 0$---entry costs are low enough that the institution forms under unanimity at all priors---then $\Pi_H^*(U, p) > \Pi_H^*(M, p)$ for every $p \in (0, 1)$.
\end{corollary}

When $\tau(U) > 0$, the comparison below the unanimity entry threshold depends on the shape of the entry set. If entry under unanimity occurs on a connected upper interval, the comparison admits a sharp characterization.
...
Suppose $\alpha < \alpha^*(N, \beta)$ and Assumption \ref{ass:monotone_entry} holds. If $\tau(U) > \tau(M)$, the institutional comparison has the \emph{single-crossing property}: there exists a unique threshold $p^*$ such that majority dominates for $p < p^*$ and unanimity dominates for $p > p^*$. The threshold has a closed-form expression (Appendix B.7). Any advantage of majority arises exclusively through the entry margin.
\end{theorem}
...
Theorem \ref{thm:crossing} resolves the institutional choice puzzle when entry is the binding constraint. Majority's only advantage is that it makes participation cheaper for weak states, because the hegemon captures less surplus. But this advantage is relevant only when the prior is pessimistic enough that entry is binding. The single-crossing property means there is no oscillation: the ranking switches at most once. Consensus is not sometimes better and sometimes worse depending on delicate parameter configurations; it is better whenever the institution is viable.
...
## B.6 Proof of Theorem 1 (Dominance of unanimity above $\tau(U)$) {-}

Fix $p \geq \tau(U)$, so entry occurs under unanimity. Since $\tau(U) \geq \tau(M)$, entry also occurs under majority. By Lemma \ref{lem:conditional}, $v(p, U) > v(p, M)$. Under majority, $v(\mu, M)$ is affine above $\tau(M)$, hence already concave, so $\operatorname{cav} v(p, M) = v(p, M)$. Therefore $\Pi_H^*(U, p) = \operatorname{cav} v(p, U) \geq v(p, U) > v(p, M) = \operatorname{cav} v(p, M) = \Pi_H^*(M, p)$. $\square$

## B.7 Proof of Theorem 2 (Single-crossing institutional comparison) {-}

\textit{Maintained assumption.} The entry set under unanimity is an upper interval: $E_U = [\tau(U), 1]$ (Assumption \ref{ass:monotone_entry}). This holds whenever the entry cost $c$ exceeds $\max_{\mu \leq \mu_s^{R1}} V_W^{R1}(\mu, \text{agg})$, so that entry does not occur in the aggressive regime (see Appendix A.7).
...
*Step 1: Concavification below $\tau(U)$.* Since $v(\mu, U) = 0$ for $\mu < \tau(U)$ and $v(0, U) = 0$, the concavification on $[0, \mu_U^*]$ (where $\mu_U^*$ achieves $S_U$) is $\operatorname{cav} v(p, U) = S_U \cdot p$. Under majority: if $\tau(M) = 0$, then $\operatorname{cav} v(p, M) = \lambda_M V_e(p)$; if $\tau(M) > 0$, then $\operatorname{cav} v(p, M) = S_M \cdot p$ for $p < \tau(M)$ and $\lambda_M V_e(p)$ for $p \geq \tau(M)$.

*Step 2: Case $\tau(M) = 0 < \tau(U)$.* For $p \in (0, \tau(U))$, $D(p) = S_U \cdot p - \lambda_M V_e(p) = [S_U - \lambda_M(r-1)]p - \lambda_M$, which is linear. $D(0) = -\lambda_M < 0$; by Theorem \ref{thm:dominance}, $D(\tau(U)) > 0$. So $D$ has a unique zero $p^* = \lambda_M/[S_U - \lambda_M(r-1)] \in (0, \tau(U))$.

*Step 3: Case $0 < \tau(M) \leq \tau(U)$, $S_U > S_M$.* For $p < \tau(M)$: $\operatorname{cav} v(p, U) = S_U p > S_M p = \operatorname{cav} v(p, M)$. For $p \in [\tau(M), \tau(U))$: $D(\tau(M)) = (S_U - S_M)\tau(M) > 0$ and $D(\tau(U)) > 0$ by Theorem \ref{thm:dominance}. Unanimity dominates globally.

*Step 4: Case $0 < \tau(M) \leq \tau(U)$, $S_U \leq S_M$.* For $p < \tau(M)$: $\operatorname{cav} v(p, U) \leq \operatorname{cav} v(p, M)$. For $p \in [\tau(M), \tau(U))$: $D(\tau(M)) \leq 0$ and $D(\tau(U)) > 0$, so $D$ crosses zero once at $p^* \in [\tau(M), \tau(U)]$.
