...
By Lemma \ref{lem:conditional}, $v(p, U) > v(p, M)$. It remains to show that persuasion cannot improve the majority payoff at $p$. Although $v(\cdot, M)$ is not globally concave because of the entry jump at $\tau(M)$, its concavification coincides with the primitive payoff for all priors above the entry threshold. Splits entirely above $\tau(M)$ do not change expected payoff because $v(\cdot, M)$ is affine on that region. Splits assigning positive mass below $\tau(M)$ strictly reduce payoff because $v = 0$ below the threshold. Hence $\operatorname{cav} v(p, M) = v(p, M)$ for every $p \geq \tau(M)$.
