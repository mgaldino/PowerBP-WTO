...
\begin{proof}
Let $\bar V_W = V_W^{R1}(1, U) = r(1-\beta\alpha)/N$. At any $\mu$, the R1 unanimity problem has two possible offer types (conservative and aggressive), each evaluated using either the low or high R2 continuation branch. We show $\bar V_W - V_W \geq 0$ for each of the four payoff candidates.
...
*Aggressive R1, high R2* ($\mu \geq \mu_s^{R2}$): After substituting $W_2^H(\mu) = [V_e(\mu)-\alpha r]/N$ and simplifying, $\bar V_W - V_W^{AH}(\mu) = [(1-\mu)(r-1) + \mu r(1-\beta)]/N > 0$ for all $\mu \in [0,1]$.
...
*Aggressive R1, low R2* ($\mu < \mu_s^{R2}$): Similarly, $\bar V_W - V_W^{AL}(\mu)$ is affine. At $\mu = 0$: $(r-1)(N-\alpha\beta)/N^2 > 0$. At $\mu = \mu_s^{R2}$: $r(r-1)(1-\alpha\beta)/[N(r-\alpha)] > 0$. Hence $\bar V_W - V_W^{AL} > 0$ on $[0, \mu_s^{R2}]$.
