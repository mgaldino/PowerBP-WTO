...
*Step 4: Case $0 < \tau(M) \leq \tau(U)$, $S_U \leq S_M$.* For $p < \tau(M)$: $\operatorname{cav} v(p, U) \leq \operatorname{cav} v(p, M)$. For $p \in [\tau(M), \tau(U))$: $\operatorname{cav} v(p, U) = S_U \cdot p$ and $\operatorname{cav} v(p, M) = \lambda_M[1 + (r-1)p]$, so $D(p)$ is affine on this interval. Since $D(\tau(M)) \leq 0$ and $D(\tau(U)) > 0$, $D$ crosses zero exactly once at some $p^* \in [\tau(M), \tau(U)]$.
