...
Let $m(p) = \operatorname{cav} v(p, M)$ and $u(p) = \operatorname{cav} v(p, U)$. Define $a = \inf E_U$.

**Preliminary: structure of $m(p)$.** Since $E_M = [\tau(M), 1]$ and $V_H^{R1}(\mu, M) = \lambda_M V_e(\mu)$ is affine, the concavified majority payoff is $m(p) = S_M \cdot p$ for $p \leq \tau(M)$ and $m(p) = \lambda_M V_e(p)$ for $p \geq \tau(M)$, where $S_M = \lambda_M V_e(\tau(M))/\tau(M)$. The ratio $m(p)/p$ is weakly decreasing on $(0, 1]$: it is constant on $(0, \tau(M)]$ and decreasing above $\tau(M)$ (affine function with positive intercept divided by $p$).

**Part 1: $p \geq a$.** Since $E_U \subseteq E_M$, we have $a \geq \tau(M)$. If $p \in E_U$, then $u(p) \geq v(p, U) > v(p, M) = m(p)$ by conditional dominance (Lemma \ref{lem:conditional}) and $p \geq \tau(M)$. If $p \notin E_U$ but $p \geq a$, then $p$ lies in a gap $(b, d)$ of $E_U$ with $b, d \in E_U$ (the gap is bounded above because $1 \in E_U$ by Lemma \ref{lem:VW_max}). Since $u(b) > m(b)$ and $u(d) > m(d)$ by the previous argument, and since $u$ is concave, $u(p)$ lies weakly above the chord connecting $(b, u(b))$ and $(d, u(d))$. Since $m$ is affine on $[b,d] \subseteq [\tau(M), 1]$ and the chord is strictly above $m$ at both endpoints, the chord is strictly above $m$ throughout $(b, d)$. Hence $u(p) > m(p)$ for all $p \geq a$.
