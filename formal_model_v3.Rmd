...
Suppose $\alpha < \alpha^*(N, \beta)$ and Assumption \ref{ass:monotone_entry} holds. The institutional comparison has the \emph{single-crossing property}: unanimity dominates at all sufficiently high priors, and the ranking changes at most once. Specifically, let $S_U$ and $S_M$ denote the persuasion slopes under unanimity and majority (defined in Appendix B.7).
\begin{itemize}
\item[\textbf{(a)}] If $S_U \geq S_M$, unanimity dominates for every $p \in (0,1]$.
\item[\textbf{(b)}] If $S_U < S_M$, there exists a unique threshold
\[
