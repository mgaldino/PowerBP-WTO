...
Ressalva de citação — episódio em duas rodadas, com correção de erro do Fable (registrar para honestidade do trilho):

1. Codex ressalvou que Kreps–Wilson assume natureza com probabilidades positivas (priors degenerados fora do domínio formal) e que "Bayes quando possível" de Fudenberg–Tirole não determina o caso 0/0. O Fable aceitou a parte de Kreps–Wilson mas contestou a de Fudenberg–Tirole, atribuindo-lhes de memória as condições "B(ii) no signaling what you don't know" e "B(iv) never dissuaded once convinced" e afirmando que a definição completa deles implicava a restrição de suporte.
2. Codex conferiu o texto literal (JET 1991 e livro §8.2.3, com auditoria independente) e refutou: os rótulos estavam errados (no numeramento deles, "no signaling..." é B(iii), não B(ii); B(iv) trata de crenças comuns sobre o tipo de terceiros); "Never Dissuaded Once Convinced" (NDOC) é condição de Osborne–Rubinstein (Bargaining and Markets, 1990), não de Fudenberg–Tirole; e, decisivamente, os próprios Fudenberg–Tirole registram que suas condições NÃO restringem a atualização após ação de probabilidade condicional zero do jogador informado — exatamente o caso 0/0. O Fable aceitou a correção integralmente; sem PDFs locais para reconferência, os registros do projeto passaram a evitar qualquer número de condição.
3. Sutileza registrada: NDOC integral seria MAIS FORTE que a decisão — tornaria absorvente qualquer posterior zero, inclusive os atingidos no interior por atualização on-path. A regra decidida é deliberadamente mais estreita: só tipos com prior zero são irrecuperáveis; no interior, crenças após desvios de H continuam livres em [0,1].

Formulação bibliograficamente segura, adotada para manuscrito e registros: "Adotamos a atualização bayesiana e a disciplina de não sinalização de Fudenberg e Tirole (1991). Nos jogos de endpoint, acrescentamos uma restrição explícita de preservação do suporte inicial: um tipo com probabilidade a priori zero nunca recebe probabilidade posterior positiva. Essa condição é motivada pela equivalência dos endpoints ao jogo de informação completa e é análoga, porém mais estreita, à lógica de não ressurreição da condição NDOC de Osborne e Rubinstein (1990)." Nunca "segue de Kreps–Wilson"; nunca números de condição sem conferência direta; nunca "adotamos NDOC integralmente". A decisão autoral em si permaneceu inalterada nas duas rodadas.
