...
Substantively, $\alpha$ measures the strength of the hegemon's bilateral outside option relative to multilateral cooperation. The threshold $\alpha^*$ identifies when the cost of buying unanimous agreement---which requires $N-1$ votes, versus $q-1$ under majority---outweighs the screening advantage. Because unanimity's vote-buying cost grows with $N$ while majority's grows more slowly, $\alpha^*$ decreases with organization size: for $\beta = 0.9$, $\alpha^* \approx 0.47$ when $N = 5$ but falls to $\approx 0.03$ when $N = 164$. The condition is therefore easy to satisfy in small institutions but demanding in large ones. For a WTO-sized organization, conditional dominance of unanimity requires that the hegemon's bilateral alternatives capture only a small fraction of the multilateral surplus. This does not eliminate the mechanism---the screening jump persists regardless of $\alpha$---but it restricts the domain over which unanimity dominates at *every* belief. Section \@ref(scope) discusses what happens outside this domain.
